...
bool OV2640_Initialization(void)
{
    ...
    if (s)
    {
        ...
        //// jwc 25-0411 ? not work for grayscale? // Custom camera configs should go here \/
        //// jwc 25-0411 ? not work for grayscale? //
        //// jwc 25-0411 ? not work for grayscale? s->set_brightness(s, 0);     // -2 to 2
        //// jwc 25-0411 ? not work for grayscale? s->set_contrast(s, 0);       // -2 to 2
        //// jwc 25-0411 ? not work for grayscale? s->set_saturation(s, 0);     // -2 to 2
        //// jwc 25-0411 ? not work for grayscale? s->set_whitebal(s, 1);       // 0 = disable , 1 = enable
        //// jwc 25-0411 ? not work for grayscale? s->set_awb_gain(s, 1);       // 0 = disable , 1 = enable
        //// jwc 25-0411 ? not work for grayscale? s->set_wb_mode(s, 0);        // 0 to 4 - if awb_gain enabled (0 - Auto, 1 - Sunny, 2 - Cloudy, 3 - Office, 4 - Home)
        //// jwc 25-0411 ? not work for grayscale? s->set_exposure_ctrl(s, 1);  // 0 = disable , 1 = enable
        //// jwc 25-0411 ? not work for grayscale? s->set_aec2(s, 1);           // 0 = disable , 1 = enable
        //// jwc 25-0411 ? not work for grayscale? s->set_ae_level(s, 0);       // -2 to 2
        //// jwc 25-0411 ? not work for grayscale? s->set_aec_value(s, 168);    // 0 to 1200
        //// jwc 25-0411 ? not work for grayscale? s->set_gain_ctrl(s, 1);      // 0 = disable , 1 = enable
        //// jwc 25-0411 ? not work for grayscale? s->set_agc_gain(s, 0);       // 0 to 30
        //// jwc 25-0411 ? not work for grayscale? s->set_gainceiling(s, (gainceiling_t)0);  // 0 to 6
        //// jwc 25-0411 ? not work for grayscale? s->set_bpc(s, 0);            // 0 = disable , 1 = enable
        //// jwc 25-0411 ? not work for grayscale? s->set_wpc(s, 1);            // 0 = disable , 1 = enable
        //// jwc 25-0411 ? not work for grayscale? s->set_raw_gma(s, 1);        // 0 = disable , 1 = enable
        //// jwc 25-0411 ? not work for grayscale? s->set_lenc(s, 1);           // 0 = disable , 1 = enable
        //// jwc 25-0411 ? not work for grayscale? s->set_hmirror(s, 1);        // 0 = disable , 1 = enable
        //// jwc 25-0411 ? not work for grayscale? s->set_vflip(s, 1);          // 0 = disable , 1 = enable
        //// jwc 25-0411 ? not work for grayscale? s->set_dcw(s, 1);            // 0 = disable , 1 = enable
