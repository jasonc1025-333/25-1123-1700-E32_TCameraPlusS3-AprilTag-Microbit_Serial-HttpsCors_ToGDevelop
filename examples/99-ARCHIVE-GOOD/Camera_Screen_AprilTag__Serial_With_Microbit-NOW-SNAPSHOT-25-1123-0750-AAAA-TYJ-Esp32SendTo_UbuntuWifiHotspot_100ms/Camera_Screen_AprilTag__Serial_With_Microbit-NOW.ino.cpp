...
const unsigned long HTTP_MIN_INTERVAL_MS = 500;
    ...
    if (OV2640_Initialization_Flag == true)
    {
        ...
        if (frame)
        {
            ...
            // Compute the transpose of the rotation matrix
                    ...
                    //// jwc 25-1121-1700 Send AprilTag data via HTTP to TestServer
                    // Get camera sensor info for sinfo->name
                    sensor_t *s = esp_camera_sensor_get();
                    if (s) {
                        camera_sensor_info_t *sinfo = esp_camera_sensor_get_info(&(s->id));
                        if (sinfo) {
                            // Send HTTP request with det->id and sinfo->name
                            bool http_success = sendAprilTagData(det->id, sinfo->name);
                            if (http_success) {
                                printf("*** HTTP: Successfully sent Tag ID %d, Camera: %s\n", det->id, sinfo->name);
                            } else {
                                printf("*** HTTP: Failed to send Tag ID %d, Camera: %s\n", det->id, sinfo->name);
                            }
                        }
                    }
