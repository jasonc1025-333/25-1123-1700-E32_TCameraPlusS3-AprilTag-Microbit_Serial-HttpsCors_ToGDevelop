...
bool OV2640_Initialization(void)
{
    ...
    //// jwc oy config.pixel_format = PIXFORMAT_RGB565; // for face detection/recognition
    config.pixel_format = PIXFORMAT_GRAYSCALE; // for april-tag detect
...
void loop()
{
    if (OV2640_Initialization_Flag == true)
    {
        ...
        if (frame)
        {
            ...
            gfx->drawGrayscaleBitmap(0, 0, (uint16_t *)frame->buf, frame->width, frame->height);
