...
void loop()
{
    ...
    while (Serial.available()) {
        ...
        Serial.printf("s.p:0< '%c' [0x%02x] ", c, c);
        printf("p:0< '%c' [0x%02x] ", c, c);
        if (++charPerLine >= 10) {
            charPerLine = 0;
            //// jwc This appears to work-NOT via Uart0 Port
            Serial.printf("s.p:\n");
    ...
    //// jwc This appears to work-NOT via Uart0 Port
    Serial.printf("s.p:%s", string_String);
    //// jwc This appears to work via Uart0 Port \/
    printf("p:%s", string_String);
  

    
    //// jwc This appears to work-NOT via Uart0 PortSerial.printf("s.p: .0.\n");
    //// jwc This appears to work via Uart0 Port \/
    printf("p: .0.\n");
